...
// body: { sectionId, selection, command, custom? }
export async function POST(req: NextRequest, { params }: Ctx) {
  const b = await req.json();
  const sectionId: string = b.sectionId;
  const selection: string = b.selection || "";
  const command: string = b.custom?.trim() || b.command || "Improve";
  if (!sectionId || !selection.trim()) return NextResponse.json({ error: "selection wajib" }, { status: 400 });
  ...
  const { system, user, allowed } = editMessages({ project, selection, command, sectionTitle: section.title, sources });
  ...
  try {
    ...
    return NextResponse.json({
      result: text,
      diff: { before: selection, after: text },
